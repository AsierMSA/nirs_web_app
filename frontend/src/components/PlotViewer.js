...
function PlotViewer({ fileName, plotData }) {
  ...
  return (
    <div className="plot-viewer">
      {/* Header con toggle para análisis avanzado */}
      <div className="plot-viewer-header">
        <h3>Analysis Results: {fileName}</h3>
        <div className="view-controls">
          <button 
            className={`view-toggle ${!showAdvanced ? 'active' : ''}`}
      ...
      {showAdvanced ? (
        <AdvancedAnalysisViewer plotData={plotData} fileName={fileName} />
      ) : (
        <div>
          {/* Display Temporal Validation Results if available */}
          {temporalValidation && (
            <div className="temporal-validation-results plot-item plot-item-medium">
